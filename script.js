...
const sliderContent = sliderContext.querySelector(".slider__content");
...
function openPlayer() {
  ...
  slider.classList.add("opem-slider");
}
...
function changeSliderContext() {
  ...
  if (slider.textContent.length >= 18) {
	const textWrap = document.createElement("span");
	textWrap.className = "text-wrap";
	textWrap.innerHTML =
...
function progressUpdate() {
  ...
  progressFilledWidth.style.width = progressFilledWidth;

  if (isPlay && this.duration == this.currentTime) {
	next();
  }

  if (count == sliderContentLenth && song.currentTime == song.duration) {
